...
class ChatClient_Tests: StressTestCase {
    ...
    func test_webSocketClient_hasAllMandatoryMiddlewares() throws {
        ...
        let typingStartCleanupMiddlewareIndex = middlewares.firstIndex {
            $0 is TypingStartCleanupMiddleware
        }
        ...
        let typingStateUpdaterMiddlewareIndex = middlewares
            .firstIndex {
            $0 is UserTypingStateUpdaterMiddleware
        }
    ...
    func test_clientProvidesConnectionId() throws {
        ...
        XCTAssertEqual(try waitFor {
            client.provideConnectionId(completion: $0)
        }, connectionId)
    ...
    func test_productionClientIsInitalizedWithAllMandatoryBackgroundWorkers() {
        ...
        XCTAssert(client.backgroundWorkers.contains {
            $0 is MessageSender
        })
        XCTAssert(client.backgroundWorkers.contains {
            $0 is NewChannelQueryUpdater
        })
        XCTAssert(client.backgroundWorkers.contains {
            $0 is NewUserQueryUpdater
        })
        XCTAssert(client.backgroundWorkers.contains {
            $0 is MessageEditor
        })
        XCTAssert(client.backgroundWorkers.contains {
            $0 is ConnectionRecoveryUpdater
        })
        XCTAssert(client.backgroundWorkers.contains {
            $0 is AttachmentUploader
        })
    ...
    func test_connectUser_setsTokenProvider_andInitiatesConnection() {
        ...
        client.userConnectionProvider?.getToken(client) {
            providedToken = try! $0.get()
        }
